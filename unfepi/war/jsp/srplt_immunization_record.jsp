...
<script type="text/javascript">
	$(function() {
		// 	$(".searchpalette, .dvwform").hide();
		$(" #addSearchFilter, #returnedDataDetails, .dvwform").hide();

		if ($("#calendersearchoption").val().length == 0) {
			$(" #addSearchFilter, #returnedDataDetails, .dvwform").hide();
		} else {
			$(" #addSearchFilter, #returnedDataDetails, .dvwform").show();
		}
...
// 					alert(JSON.stringify(srfilters[i]));
					...
					$('#' + srfilters[i].cname_id).val() + " "+ $('#' + srfilters[i].ftype_id).val();
...
// 			console.log(sf.length);
	...
	<table width="100%">
		...
		<tr>
			<td class="left">
				<div id="returnedDataDetails" class="returnedDataDetails">
					<a onclick="$('#winhelp').window('open');" class="linkiconM iconhelp leftalign"></a><br><br></div>
				<div id="sfdiv"></div>
			</td>
			<td rowspan="10" class="right"><span class="left">${model.searchFilterMessage}</span>
			<td>
		</tr>
		<tr>
			<td class="left">
			...
			<c:set var="nextSearchFieldNameValue" value="<%=SearchFilter.CALENDER.FILTER_NAME()%>"></c:set>
			<select id="calendersearchoption" name="${nextSearchFieldNameValue}" bind-value="${model[nextSearchFieldNameValue]}">
			   	<option value=""></option>
				<c:forEach items="${model.vaccinationcalendar}" var="cal">
					<option value="${cal.calenderId}">${cal.shortName}</option>
				</c:forEach>
			</select>
			
			<a id="sendSearchFilter" onclick="sendSearchFilter();" class="anchorCustom">Search</a> 
			<a id="addSearchFilter" onclick="addSearchFilter();" class="anchorCustom">Add filter</a> 
			...
			<input id="columns" type="hidden" name="${nextSearchFieldNameValue}" value="${model[nextSearchFieldNameValue]}"></td>
<%-- 	 	<td><a href="viewDownloadables.htm?action=search&<%=SearchFilter.NAME_PART.FILTER_NAME()%>=${model.dmpFileNameInit}&<%=SearchFilter.TYPE.FILTER_NAME()%>=<%=DownloadableType.DATA_DUMP%>" class="linkiconM iconcsv"></a></td> --%>
	...
	<div id="winhelp" class="dvwform">
		<table>
			...
			<tr><td>ChildID</td><td>8 digits</td></tr>
			<tr><td>Gender</td><td>Male | Female</td></tr>
			<tr><td>IsEstimatedBirthdate</td><td>Yes | No</td></tr>
			<tr><td>VaccinationStatus</td><td>Vaccinated | Retro | Retro_date_missing</td></tr>
