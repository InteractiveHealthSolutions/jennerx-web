...
<table class="filter">
	...
	<tr>
		<td><select id="search_hp" name="search_healthprogram"
			bind-value="">
				<option></option>
		...
		</select> <c:forEach items="${model.roundList}" var="rd">
				<input id="rd_sd_${rd.roundId}" type="hidden"
					value="<fmt:formatDate value="${rd.startDate}" pattern="<%=WebGlobals.GLOBAL_DATE_FORMAT_JAVA%>"/>" />
				<input id="rd_ed_${rd.roundId}" type="hidden"
					value="<fmt:formatDate value="${rd.endDate}" pattern="<%=WebGlobals.GLOBAL_DATE_FORMAT_JAVA%>"/>" />

			</c:forEach></td>
		<td><select id="search_st" name="search_site" bind-value="">
				<option></option>
				<c:forEach items="${model.siteList}" var="st">
					<option id="st_${st.mappedId}" value="${st.mappedId}" disabled>${st.name}</option>
				</c:forEach>
		</select></td>
		<td><input id="search_cl" name="search_date"
			placeholder='dd-MM-yyyy' /></td>
		<td><button onclick="filterDailyReport();">search</button></td>
	</tr>
</table>
<h1 id="calendername"
	style="float: right; padding-right: 100px; color: #ED8117"></h1>
...
$( document ).ready(function() {
	...
	$("#search_rd").change(function(){
		$("#search_st, #search_cl").val("");
	});
	
	$("#search_hp").change(function(){
		
		$("#search_rd, #search_st, #search_cl").val("");
		$("#search_rd option, #search_st option").prop('disabled', true);
