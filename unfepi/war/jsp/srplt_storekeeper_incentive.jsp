...
<table>
 	<tr>
    <td>Storekeeper<br>
    <c:set var="nextSearchFieldNameValue" value="<%=SearchFilter.STOREKEEPER.FILTER_NAME()%>"></c:set>
	<input type="hidden" id="storekeeperIdinh" value="${model[nextSearchFieldNameValue]}"/> 
    <select id="storekeeperddp" name="${nextSearchFieldNameValue}">
        <option></option>
        <c:forEach items="${model.storekeepers}" var="storekeeper"> 
        <option value="${storekeeper.mappedId}">${storekeeper.idMapper.identifiers[0].identifier}:${storekeeper.firstName}</option>
    	</c:forEach> 
    </select>
    <script><!--
       sel = document.getElementById("storekeeperddp");
       val=document.getElementById("storekeeperIdinh").value;
