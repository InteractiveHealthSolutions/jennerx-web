...
public class ControllerUIHelper {
	...
	/**
	 * Manipulates (populates required default info) and saves data for entities
	 * participating in enrollment for lottery generator half filled forms. All
	 * values MUST have been validated via
	 * {@link ValidatorUtils#validateEnrollmentFillLotteryGenForm} before
	 * calling the method. Changes status for Child and vaccinations, creates
	 * default Measles1 if needed, handles next vaccination if required.
	 * 
	 * @see {@linkplain ControllerUIHelper#handleNewNextVaccination} AND
	 *      {@linkplain ControllerUIHelper#handleEnrollmentPreference} AND
	 *      {@linkplain ControllerUIHelper#handleEnrollmentContactInfo} AND
	 *      {@linkplain IMRUtils#createMeasles1}
	 * @param child
	 *            MUST have been validated using
	 *            {@link ValidatorUtils#validateEnrollmentFillLotteryGenForm}
	 * @param enrollmentVaccination
	 *            MUST have been validated using
	 *            {@link ValidatorUtils#validateEnrollmentFillLotteryGenForm}
	 * @param nextVaccine
	 ...
	 * @throws ChildDataInconsistencyException
	 */
	public static void doEnrollmentLotteryGen(Child child, Vaccination enrollmentVaccination, Vaccine nextVaccine, Vaccination nextVaccinationObject, LotterySms preference, ContactNumber primContact,
			ContactNumber secContact, Address address, User user, ServiceContext sc) throws ChildDataInconsistencyException {
		/*
		 * child.setCreator(user); child.setStatus(STATUS.FOLLOW_UP);
		 * sc.getChildService().updateChild(child);
		 * 
		 * enrollmentVaccination.setCreator(user);
		 * sc.getVaccinationService().updateVaccinationRecord
		 * (enrollmentVaccination);
		 * 
		 * Vaccine currentVaccine =
		 * IMRUtils.getVaccineObject(enrollmentVaccination, sc);
		 * 
		 * //if none of the vacc is measles then create vaccination and
		 * reminders automatically if(
		 * !currentVaccine.getName().toLowerCase().contains("measles") &&
		 * !nextVaccine.getName().toLowerCase().contains("measles") ){
		 * IMRUtils.createMeasles1(child.getMappedId(), child.getBirthdate(),
		 * child.getDateEnrolled(),
		 * enrollmentVaccination.getVaccinationCenterId(),
		 * preference.getHasApprovedReminders(), user, sc); }
		 * 
		 * // Only case when next vaccine could be null and not applicable is
		 * that current vaccine is M2
		 * if(!currentVaccine.getName().toLowerCase().contains("measles2")){
		 * handleNewNextVaccination(child, enrollmentVaccination, nextVaccine,
		 * nextVaccinationObject, preference.getHasApprovedReminders(), user,
		 * sc); }
		 * 
		 * handleEnrollmentPreference(preference, currentVaccine, child, user,
		 * sc);
		 * 
		 * handleEnrollmentContactInfo(preference.getHasApprovedReminders(),
		 * primContact, secContact, address, child, user, sc);
		 */}
	...
	/**
	 * Manipulates (populates required default info) and saves data for entities
	 * participating in followup. All values MUST have been validated via
	 * {@link ValidatorUtils#validateFollowupForm} before calling the method.
	 * 
	 * @param vaccineSchedule
	 * @see {@linkplain ControllerUIHelper#handleFollowupVaccinations} AND
	 *      {@linkplain ControllerUIHelper#handleFollowupPreference}
	 * @param child
	 *            MUST have been validated using
	 *            {@link ValidatorUtils#validateFollowupForm}
	 * @param currentVaccination
	 *            MUST have been validated using
	 *            {@link ValidatorUtils#validateFollowupForm}
	 * @param nextVaccine
	...
	public static void doWomenFollowup(DataEntrySource dataEntrySource, WomenVaccinationCenterVisit centerVisit, Date formStartDate, Women women, Address address, User user, ServiceContext sc)
			throws VaccinationDataException, ChildDataInconsistencyException {
	...
	/**
	 * Manipulates (populates required default info) and saves data for entities
	 * participating in admin/privileged followup. All values MUST have been
	 * validated via {@link ValidatorUtils#validateFollowupPrivilegedForm}
	 * before calling the method.
	 * 
	 * @see {@link ValidatorUtils#handleNewNextVaccination} AND
	 *      {@link ValidatorUtils#handleExistingNextVaccination}
	 *      {@link ValidatorUtils#handleFollowupPreference}
	 * @param child
	 *            MUST have been validated using
	 *            {@link ValidatorUtils#validateFollowupPrivilegedForm}
	 * @param currentVaccination
	 *            MUST have been validated using
	 *            {@link ValidatorUtils#validateFollowupPrivilegedForm}
	 * @param nextVaccine
	 ...
	 * @throws ChildDataInconsistencyException
	 */
	public static void doFollowupPrivileged(Child child, Vaccination currentVaccination, Vaccine nextVaccine, Vaccination nextVaccinationObject, boolean hasApprovedReminders,
			String reminderCellNumber, User user, ServiceContext sc) throws VaccinationDataException, ChildDataInconsistencyException {
		/*
		 * currentVaccination.setCreator(user);
		 * 
		 * Vaccination previousVaccination =
		 * getPreviousVaccination(currentVaccination, sc);
		 * currentVaccination.setIsVaccinationCenterChanged
		 * (previousVaccination==
		 * null?null:(currentVaccination.getVaccinationCenterId()!=
		 * previousVaccination.getVaccinationCenterId()));
		 * currentVaccination.setDescription
		 * ((currentVaccination.getDescription()
		 * ==null?"":currentVaccination.getDescription
		 * ())+WebGlobals.GLOBAL_JAVA_DATETIME_FORMAT.format(new
		 * Date())+"VIA-ADMIN_FOLLOWUP");
		 * 
		 * currentVaccination.setVaccinationRecordNum(Integer.parseInt(sc.
		 * getVaccinationService
		 * ().addVaccinationRecord(currentVaccination).toString()));
		 * 
		 * Vaccine currentVaccine =
		 * IMRUtils.getVaccineObject(currentVaccination, sc); boolean
		 * receivedVaccineIsP3OrM2 =
		 * (currentVaccine.getName().toLowerCase().contains("measles2") ||
		 * currentVaccine.getName().toLowerCase().contains("penta3")); // next
		 * vaccine can only be NULL if current vaccine is P3 or M2 or
		 * nextVaccine has already been received // i.e. if currentVaccine is
		 * neither P3 nor M2 then must do it // otherwise if nextVaccine IS NOT
		 * NULL then must do it, // but if nextVaccine is NULL user should
		 * deliberately ask to ignore nextVaccine if(!receivedVaccineIsP3OrM2 ||
		 * nextVaccine != null ||
		 * !ValidatorUtils.IGNORE_NEXT_VACCINE_FOLLOWUP_PRIVILEGED_FORM){
		 * nextVaccinationObject =
		 * sc.getVaccinationService().getPendingVaccination
		 * (currentVaccination.getChildId(), nextVaccine.getName(), false,
		 * null);
		 * 
		 * if(nextVaccinationObject == null){ nextVaccinationObject = new
		 * Vaccination();
		 * 
		 * handleNewNextVaccination(child, currentVaccination, nextVaccine,
		 * nextVaccinationObject, hasApprovedReminders, user, sc); } else {
		 * handleExistingNextVaccination(currentVaccination,
		 * nextVaccinationObject, hasApprovedReminders, user, sc); } }
		 * 
		 * handleFollowupPreference(currentVaccination, hasApprovedReminders,
		 * reminderCellNumber, user, sc);
		 */}
	...
	/**
	 * Manipulates (populates required default info) and saves data for entities
	 * participating in storekeeper registration. All values MUST have been
	 * validated via {@link ValidatorUtils#validateStorekeeperRegistrationForm}
	 * AND {@link ValidatorUtils#validateLoginCredentials} before calling the
	 * method.
	 * 
	 * @param idMapper
	 *            MUST have been validated using
	 *            {@link ValidatorUtils#validateStorekeeperRegistrationForm}
	 * @param storekeeper
	 *            MUST have been validated using
	 *            {@link ValidatorUtils#validateStorekeeperRegistrationForm}
	 * @param usernameGiven
	 ...
	 *            If for any reason Storekeeper`s username and password should
	 *            not be created
	 * @param user
	 *            Authenticated user who had done dataEntry
	 * @param sc
	...
	private static void handleFollowupWomenVaccinations(DataEntrySource dataEntrySource, WomenVaccinationCenterVisit centerVisit, User user, ServiceContext sc) throws VaccinationDataException {
		...
		if (tt1.getVaccinationRecordNum() == 0 && !tt1.getVaccinationStatus().equals(WOMEN_VACCINATION_STATUS.NOT_VACCINATED)) {
		...
		if (tt2.getVaccinationRecordNum() == 0 && !tt2.getVaccinationStatus().equals(WOMEN_VACCINATION_STATUS.NOT_VACCINATED)) {
		...
		if (tt3.getVaccinationRecordNum() == 0 && !tt3.getVaccinationStatus().equals(WOMEN_VACCINATION_STATUS.NOT_VACCINATED)) {
		...
		//sc.getWomenVaccinationService().updateVaccinationRecord(centerVisit);
	}
	...
	private static void handleWomenFollowupContactInfo(String primaryContact, String secondaryContact, Address address, Women women, User user, ServiceContext sc)
		...
		for(ContactNumber contact : number){
			// if else because for now there are only two types of contact numbers
	...
	// TODO
	/**
	 * NEW Manipulate and save IdMapper for role CHILD and save child for ID
	 * assigned by IdMapper
	 */
	...
	public static void prepareVaccinationReferenceData(HttpServletRequest request, Map<String, Object> model, List<VaccinationCenter> centeres, List<Vaccinator> vaccinators) {
		/*
		 * SortedMap<String, List<Vaccine>> vaccineMap = new TreeMap<String,
		 * List<Vaccine>>(); for (Vaccine vaccine : vaccines) { //if this
		 * group(Description represents group) doesnot exists in map add it as a
		 * key and add vaccine in the newly created list
		 * if(!vaccineMap.containsKey(vaccine.getDescription())){ List<Vaccine>
		 * vl = new ArrayList<Vaccine>(); vl.add(vaccine);
		 * vaccineMap.put(vaccine.getDescription(), vl); } else{ //Else add
		 * vaccine to the list already existing for the group.
