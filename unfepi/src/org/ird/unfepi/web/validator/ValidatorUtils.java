...
****************************************************	 * 
			...
			HashMap<String, String> mobileErrors, Errors webErrors, ServiceContext sc)
	{
		...
		validateVaccinationSchedule(dataEntrySource, sc.getChildService().findChildById(centerVisit.getChildId(), true, null), false, centerVisit, vaccineSchedule, mobileErrors, webErrors, sc, useFieldPrefix);
			...
			String birthdateOrAge, String ageYears, HashMap<String, String> mobileErrors, Errors webErrors, ServiceContext sc, boolean useFieldPrefix)
	{
		...
		if(StringUtils.isEmptyOrWhitespaceOnly(birthdateOrAge)
				|| (!birthdateOrAge.equalsIgnoreCase("age") && !birthdateOrAge.equalsIgnoreCase("birthdate"))){
	 ...
	 * @param dataEntrySource : MUST be specified
	 * @param address : 
	 * <ul><li>address1, landmark- optional but if provided must not have special characters other than( |/()_,-.(space)` )
	...
	private static void validateVaccinationSchedule(DataEntrySource dataEntrySource, Child child, boolean isNewEnrollment, VaccinationCenterVisit centerVisit, 
			List<VaccineSchedule> vaccineSchedule, HashMap<String, String> mobileErrors, Errors error, ServiceContext sc, boolean useFieldPrefix)
	{
		...
		// as all fields are for event information of same datetime
			...
			return;
    			...
    			&& (!measles2Given  && DateUtils.differenceBetweenIntervals(centerVisit.getVisitDate(), child.getBirthdate(), TIME_INTERVAL.YEAR) > MAX_AGE_YEARS_OTHER_VACCINES) 
	...
	*/
			...
			HashMap<String, String> mobileErrors, Errors error, ServiceContext sc, boolean useFieldPrefix)
	{
    	...
    	else
    	{
	    	...
	    	if(!vaccination.getVaccinationStatus().equals(VACCINATION_STATUS.RETRO_DATE_MISSING) 
    		...
    		if(vaccination.getVaccinationStatus().equals(VACCINATION_STATUS.VACCINATED)
    				&& vaccination.getVaccinationCenterId() != null){
    			ValidatorOutput vepi = null;
    			if(vaccination.getChildId() != null){
